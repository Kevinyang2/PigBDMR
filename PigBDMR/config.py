...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--deterministic", action="store_true",
                            help="Enable deterministic PyTorch/CUDA settings for reproducibility checks")
        parser.add_argument("--deterministic_strict", action="store_true",
                            help="Raise instead of warning when a nondeterministic operation is used")
        ...
        parser.add_argument('--use_null_gate', action="store_true",
                            help="Enable query-conditioned gating on the T2V cross-attention residual")
        parser.add_argument('--use_pv_repr', action="store_true",
                            help="Enable L_repr cross-moment consistency loss in PV module")
        parser.add_argument('--use_pv_adj', action="store_true",
                            help="Enable boundary adjustment in PV module")
        parser.add_argument('--use_temporal_icg_icr', action="store_true",
                            help="Enable score-only temporal context generation/refinement in PV module")
        parser.add_argument('--ticg_gt_thd', type=float, default=0.5,
                            help="Training tIoU threshold for selecting temporal context proposals")
        parser.add_argument('--ticg_score_thd', type=float, default=0.5,
                            help="Inference score threshold for selecting temporal context proposals")
        parser.add_argument('--ticg_topk', type=int, default=8,
                            help="Maximum temporal context proposals per sample")
        parser.add_argument('--use_pv_rerank', action="store_true",
                            help="Use PV scores to rerank proposals at inference")
        ...
        parser.add_argument('--fusion_variant', default='baseline', type=str, choices=('baseline', 'gated_xattn', 'film', 'gated_xattn_film'),
                            help="T2V fusion variant: baseline keeps existing cross-attention; gated_xattn gates the attention residual; film applies query-conditioned FiLM after T2V; gated_xattn_film combines both")
        parser.add_argument('--fusion_mode', default='full', type=str, choices=('add', 'add_with_dummy', 'concat', 'self_attn', 'vanilla_cross_attn', 'vanilla_cross_attn_encoder', 'cross_attn', 'full'),
                            help="Fusion strategy: "
                                 "add (mean-pool real text, add to each video token), "
                                 "add_with_dummy (mean-pool virtual+real text, legacy add baseline), "
                                 "concat (feature-dim concat [video, text_mean] per token, project), "
                                 "self_attn (sequence concat video+text, self-attn on combined seq), "
                                 "vanilla_cross_attn (standard cross-attn, no Virtual Encoder gating, no post encoder), "
                                 "vanilla_cross_attn_encoder (standard cross-attn + fusion encoder), "
                                 "cross_attn (adaptive T2V cross-attn with Virtual Encoder gating, no post encoder), "
                                 "full (adaptive T2V cross-attn + fusion encoder)")
        parser.add_argument('--dummy_layers', default=2, type=int,
                            help="Number of Virtual Encoder layers")
        ...
        parser.add_argument('--num_dummies', '--num_virtual_tokens', dest='num_dummies', default=45, type=int,
                            help="Number of Virtual Encoder tokens. --num_dummies is kept as the legacy option name")
        ...
        parser.add_argument("--lw_ms_align", type=float, default=1.,
                            help="weight for saliency loss, set to 0 will ignore")
        parser.add_argument("--lw_pv", type=float, default=9.0,
                            help="weight for PV/SRM score loss (L_PV = |p - tIoU|^2)")
        parser.add_argument("--lw_pv1", type=float, default=0.7,
                            help="weight for L_repr cross-moment consistency loss (CE)")
        parser.add_argument("--lw_pv_adj", type=float, default=1.0,
                            help="weight for PV boundary adjustment loss")
        parser.add_argument("--lw_null_gate", type=float, default=1.0,
                            help="weight for Video Null Gate auxiliary loss")
        ...
        if isinstance(self, TestOptions):
            ...
            for arg in saved_options:  # use saved options to overwrite all BaseOptions args.
                if arg not in ["results_root", "num_workers", "nms_thd", "debug",  # "max_before_nms", "max_after_nms"
                               ...
                               "use_pv_rerank", "ticg_score_thd", "ticg_topk"]:
            ...
            save_fns = ['PigBDMR/model.py', 'PigBDMR/transformer.py']
