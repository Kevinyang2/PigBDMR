...
from PigBDMR.span_utils import span_xx_to_cxw
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Fallback: pig dataset uses multi_{scene_id}.npz
                            try:
                                scene_id = vid.split("_")[0]
                                _feat_path = join(_feat_dir, f"multi_{scene_id}.npz")
                                _feat = np.load(_feat_path)["features"][:self.max_v_l].astype(np.float32)
                            except:
                                raise FileNotFoundError(f"Cannot find feature for vid {vid} in {_feat_dir}")
                if self.normalize_v:
                    _feat = l2_normalize_np_array(_feat)
                v_feat_list.append(_feat)
            # some features are slightly longer than the others
