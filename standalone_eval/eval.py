...
def compute_mr_ap_by_gt(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
                        max_gt_windows=None, max_pred_windows=10, num_workers=1, chunksize=50):
    ...
    group_mean_ap_dict["1_tgt_mAP"] = 0.0
    group_mean_ap_dict["2_tgt_mAP"] = 0.0
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        if len(_ground_truth) == 0:
            ...
            group_mean_ap_dict = compute_mr_ap_by_gt(_submission, _ground_truth, num_workers=1, chunksize=50)
            ...
            ret_metrics[name] = {"MR-mIoU@1": mIoU_at_1,
                                 "MR-mIoU@2": mIoU_at_2,
                                 "MR-mIoU@3": mIoU_at_3,
                                 "MR-mAP": {"1_tgt_mAP": group_mean_ap_dict["1_tgt_mAP"], 
                                            "2_tgt_mAP": group_mean_ap_dict["2_tgt_mAP"], 
                                            ">=3_tgt_mAP": group_mean_ap_dict[">=3_tgt_mAP"],
                                            'Generalized_mAP': group_mean_ap_dict['Generalized_mAP']},
...
def mk_gt_scores(gt_data, clip_length=2):
    ...
    if "saliency_scores" in gt_data and "relevant_clip_ids" in gt_data:
